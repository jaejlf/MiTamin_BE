...
* 2022.09.27.화 : 마이타민 섭취(하루 진단, 칭찬 처방), 최근 마이타민 API 구현, 오류 구별을 위한 errorCode 추가
* 2022.09.29.목 : 웰컴 메세지 API -> 마이타민 섭취 여부에 따른 웰컴 메세지 구현

.참고사항
* 하루 진단하기, 칭찬 처방하기 API의 path가 변경되었습니다.
* DB 구조 변경으로 한차례 더 DB 삭제가 있었습니다. (당분간 테이블 변경이 조금씩 있을 것 같아서 기존 아이디 로그인 안될 경우에는 DB 비워진 상태이니 재가입해주세요.)
...
.참고사항
JWT 인증 관련 자세한 내용 : 노션 Server 공유 페이지 -> '인증 관련 공통 참고 사항'
...
== Home
=== 웰컴 메세지
* 조건에 따라 웰컴 메시지를 리턴한다. 조건 1, 2, 3 순으로 높은 우선 순위가 적용된다.
* 조건 1. 마이타민 섭취 완료 : 오늘 하루도 수고 많았어요 :)
* 조건 2. 마이타민 섭취 지정 시간 및 섭취 여부에 따른 웰컴 메세지
...
* 24시간 이내 생성된 마이타민일 경우에 canEdit 속성값은 true이다.
