...
=== 월간 마음 컨디션 조회
* 조회하는 month의 모든 날짜(1일 ~ 해당 월의 마지막날)에 대해 '일자:mentalConditionCode' 형태로 데이터를 리턴한다.
* 해당하는 날짜에 하루 진단 기록이 존재하지 않을 경우, mentalConditionCode는 '0'이다.
* mentalConditionCode
    ** 1 : "매우 나빠요 .."
    ** 2 : "나쁜 편이에요 :("
...
operation::report/getMonthlyMentalReport[snippets='http-request,http-response']
operation::report/getMonthlyMentalReport[snippets='request-headers,path-parameters,response-fields']
...
=== 칭찬 처방 랜덤 조회
* 칭찬 처방 기록이 하나도 없는 경우 data는 null로 리턴된다. (ex. data : null)
